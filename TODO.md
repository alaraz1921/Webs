...
- [ ] Sustituir los `alert()` restantes por modales propios para mantener una UX consistente.
...
- [ ] Revisar `index_redireccion.html` y confirmar si todavia tiene uso.
...
- [ ] Crear una checklist manual de pruebas para portada, ValentinaPlay, Bingo, Impostor, Privado y 404.
- [ ] Normalizar nombres de archivos y titulos visibles: por ejemplo `indeximpostor.html` podria ser `index.html` dentro de `impostor/`.
...
- [ ] Probar enlaces a `Privado`, `ValentinaPlay`, `Bingo/carton.html`, `Bingo/monitor.html` e `impostor/indeximpostor.html`.
