...
create table if not exists public.escapetin_challenges (
  ...
  description text,
  image_url text,
  challenge_type text not null default 'question' check (challenge_type in ('question', 'multiple_choice', 'qr', 'manual', 'photo')),
  question text,
  correct_answer text,
  keyword text,
  option_a text,
  ...
  qr_token text unique default public.escapetin_random_hex(12),
...
alter table public.escapetin_progress add column if not exists wrong_attempts integer not null default 0;
...
create index if not exists escapetin_admin_notifications_user_unread_idx on public.escapetin_admin_notifications (admin_user_id, read_at, created_at desc);
...
create policy escapetin_challenges_admin_all on public.escapetin_challenges
  for all to authenticated
  using (public.can_manage_escapetin() and exists (select 1 from public.escapetin_games g where g.id = game_id and g.created_by = auth.uid()))
  with check (public.can_manage_escapetin() and exists (select 1 from public.escapetin_games g where g.id = game_id and g.created_by = auth.uid()));
...
create policy escapetin_admin_notifications_own_select on public.escapetin_admin_notifications
  for select to authenticated
  using (admin_user_id = auth.uid());

drop policy if exists escapetin_admin_notifications_own_update on public.escapetin_admin_notifications;
create policy escapetin_admin_notifications_own_update on public.escapetin_admin_notifications
...
begin
  ...
  if not ok then
    if c.challenge_type = 'multiple_choice' then
      ...
      returning * into p;
      return jsonb_build_object('correct', false, 'message', 'Respuesta incorrecta, intentalo de nuevo.', 'wrong_attempts', p.wrong_attempts, 'error_penalty', floor(c.points * 0.25)::int);
    end if;
  ...
  if c.challenge_type in ('manual', 'photo') or c.requires_admin_validation then
    if g.created_by is not null then
      ...
      values (g.created_by, g.id, t.id, c.id, p.id, 'Prueba pendiente de revision', 'El equipo "' || t.name || '" ha enviado la prueba "' || c.title || '" de "' || g.title || '".');
    end if;
    return jsonb_build_object('correct', true, 'message', 'Prueba enviada. Queda pendiente de revision del administrador.', 'pending', true, 'progress_id', p.id);
  end if;
